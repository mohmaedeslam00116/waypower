...
- [Reference extractions — dual-axis-review, waymap, design-interview](t11-reference-extractions.md): pure-move references shipped (REVIEW-DISPATCH.md 12-smell catalog; MAP-SCHEMA.md map/ticket templates + label convention; SPEC-GATE.md spec stage); ≤2-line pointers, evals untouched, gate green. Audit drift corrected at extraction (12 smells not 14; no inline prompts; 4 ticket types/spec bullets); docs-page sync queued to t12.
- [Deepening cleanup — example, dedupe, docs sync](t12-deepening-cleanup.md): tracer-plan EXAMPLE-PLAN.md authored (3-task worked plan behind a one-line pointer); domain-glossary ADR criteria + file structure each stated once (references keep write-time examples); authoring-skills points at validate-evals.mjs; run log kept with exemption note as t05 evidence. Docs sync: 7 pages read; two stale-since-t05 notes fixed (12/13 → 13/13), the rest drift-free.
